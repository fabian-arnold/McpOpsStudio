  ...
  if (!entries.length)
    return (
      <p className="rounded-lg border p-3 text-xs text-muted-foreground">
        Add properties to the input schema to generate form fields. Raw JSON remains
        available.
      </p>
    );
    ...
    <div className="grid gap-3 md:grid-cols-2">
      {entries.map(([name, rawProperty]) => {
        ...
        return (
          <label
            key={name}
            className={type === "object" || type === "array" ? "md:col-span-2" : ""}
          >
            ...
            {options ? (
              <select
                ...
                onChange={(event) => updateProperty(name, property, event.target.value)}
              >
              ...
              <textarea
                className="field min-h-20 font-mono text-[11px]"
...
export function SchemaDefinitionEditor({
  ...
  function infer() {
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : "Example must be valid JSON.",
      );
    }
  }
        ...
        {!allowAny && (
          <>
            <button
              ...
              className={`rounded px-2 py-1 text-[10px] ${mode === "schema" ? "bg-muted" : "text-muted-foreground"}`}
            >
            ...
            <button
              ...
              className={`rounded px-2 py-1 text-[10px] ${mode === "example" ? "bg-muted" : "text-muted-foreground"}`}
            >
          ...
          onChange={(event) => {
            ...
            onChange(
              event.target.checked
                ? "{}"
                : previousSchema.current || defaultObjectSchema(),
            );
          }}
        />
        <span>
          <strong>Allow any</strong>
          <span className="ml-1 text-muted-foreground">
            Skip schema constraints for this value.
          </span>
        </span>
      </label>
      {allowAny ? (
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 p-3 text-[11px] text-amber-700 dark:text-amber-300">
          Any valid JSON value is accepted. Runtime schema validation is intentionally
          unrestricted.
        </div>
      ) : mode === "schema" ? (
        <>
          <textarea
            className="field min-h-52 font-mono text-[10px]"
            value={value}
            onChange={(event) => onChange(event.target.value)}
          />
          <Button size="sm" variant="ghost" className="mt-1" onClick={generate}>
            <WandSparkles size={11} /> Generate example
          </Button>
        </>
      ) : (
        <>
          ...
          />
          <Button size="sm" variant="secondary" className="mt-1" onClick={infer}>
            <WandSparkles size={11} /> Infer schema from example
          </Button>
        </>
      )}
...
export function generateExampleFromSchema(schema: JsonSchema): unknown {
  ...
  if (Array.isArray(schema.oneOf) && schema.oneOf.length)
    return generateExampleFromSchema(record(schema.oneOf[0]));
  if (Array.isArray(schema.anyOf) && schema.anyOf.length)
    return generateExampleFromSchema(record(schema.anyOf[0]));
  switch (schemaType(schema)) {
    case "object":
      return Object.fromEntries(
        Object.entries(record(schema.properties)).map(([name, child]) => [
          name,
          generateExampleFromSchema(record(child)),
        ]),
      );
    case "array":
      return [generateExampleFromSchema(record(schema.items))];
    case "boolean":
      return false;
    case "integer":
    case "number":
...
export function inferSchemaFromExample(value: unknown): JsonSchema {
  ...
  if (Array.isArray(value))
    return {
      type: "array",
      items: value.length ? inferSchemaFromExample(value[0]) : {},
    };
  if (typeof value === "object") {
    ...
    return {
      ...
      properties: Object.fromEntries(
        entries.map(([name, child]) => [name, inferSchemaFromExample(child)]),
      ),
