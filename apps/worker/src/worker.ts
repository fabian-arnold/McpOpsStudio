...
worker.on("completed", (job) => console.log(JSON.stringify({ level: "info", message: "deployment job completed", jobId: job.id })));
worker.on("failed", (job, error) => console.error(JSON.stringify({ level: "error", message: "deployment job failed", jobId: job?.id, error: error.message })));
