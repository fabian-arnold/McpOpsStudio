...
import { functionIdentifierWhere, endpointIdentifierWhere, endpointListWhere } from "./repository.js";
...
describe("route identifier filters", () => {
  ...
  it("maps friendly identifiers without passing them to UUID columns", () => {
    expect(endpointIdentifierWhere("customer-operations")).toEqual({ slug: "customer-operations" });
  ...
  it("keeps endpoint filters inside the authenticated project scope", () => {
    expect(endpointListWhere("org-session", { environmentId: "environment", status: "deployed", q: "customer" })).toMatchObject({
      projectId: "org-session", environmentId: "environment", status: "deployed",
      OR: expect.any(Array),
    });
  });
});
