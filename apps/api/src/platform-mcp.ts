...
const editSchema = z
  .object({
    ...
    changes: functionCreateSchema
      .partial()
      .omit({ code: true, secretGrantIds: true })
      .optional(),
...
const tools = [
  ...
  tool(
    "project_select",
    "Select the project used by this MCP session.",
    { project: stringField("Project ID or slug") },
    false,
  ),
  tool("project_get", "Inspect the selected project and environments.", {}, true),
  tool(
    "functions_list",
    "List Functions in the selected project.",
    { query: optionalString("Optional name, slug, or description filter") },
    true,
  ),
  tool(
    "function_get",
    "Read Function source, schemas, policy, version, and checksum.",
    { function: stringField("Function ID, slug, or name") },
    true,
  ),
  tool(
    "function_create",
    "Preview or create an immutable development Function version. Defaults to dry run.",
    {
      draft: objectField("Function draft matching the platform Function contract"),
  ...
  tool(
    "function_edit",
    "Apply a guarded unified diff or full source replacement plus optional metadata changes. Defaults to dry run.",
    {
      function: stringField("Function ID, slug, or name"),
      expectedVersion: numberField("Version returned by function_get"),
      expectedChecksum: stringField("SHA-256 returned by function_get"),
      patch: optionalString("Unified source diff"),
      source: optionalString("Complete replacement source"),
      changes: optionalObject("Partial metadata, schemas, and policy fields"),
      dryRun: booleanField("Preview without saving", true),
    },
    false,
  ),
  tool(
    "function_validate",
    "Compile and statically validate a saved Function without executing it.",
    { function: stringField("Function ID, slug, or name") },
    true,
  ),
  tool(
    "function_test",
    "Validate by default; set dryRun=false to execute the saved development Function through the isolated runtime.",
    {
      function: stringField("Function ID, slug, or name"),
  ...
  tool(
    "library_get",
    "Read a project library version and source.",
    { library: stringField("Library ID or import path") },
    true,
  ),
  tool(
    "library_create_version",
    "Preview or create a validated project library version using a patch or replacement source.",
    {
      library: optionalString("Existing library ID or import path"),
      draft: optionalObject("Required metadata for a new library"),
      patch: optionalString("Unified source diff"),
      source: optionalString("Complete replacement source"),
      dryRun: booleanField("Preview without saving", true),
    },
  ...
  tool(
    "endpoints_list",
    "List MCP and HTTP endpoints in the selected project.",
    {},
    true,
  ),
  tool(
    "endpoint_get",
    "Inspect endpoint settings and binding tables.",
    { endpoint: stringField("Endpoint ID or slug") },
    true,
  ),
  tool(
    "binding_create",
    "Preview or create an MCP tool or HTTP route binding.",
    {
      endpoint: stringField("Endpoint ID or slug"),
      binding: objectField("Typed MCP or HTTP binding"),
      dryRun: booleanField("Preview without saving", true),
    },
    false,
  ),
  tool(
    "binding_edit",
    "Preview or edit an existing binding without deleting it.",
    {
      endpoint: stringField("Endpoint ID or slug"),
      bindingId: stringField("Binding ID"),
      changes: objectField("Typed binding changes"),
      dryRun: booleanField("Preview without saving", true),
    },
    false,
  ),
  tool(
    "endpoint_discover",
    "Return final environment URLs, bindings, credential requirements, and optionally a generated client document.",
    {
      endpoint: stringField("Endpoint ID or slug"),
      format: optionalString(
        "openapi-json, openapi-yaml, postman, mcp-client, manifest-json, or manifest-yaml",
      ),
    },
    true,
  ),
  ...
  tool(
    "development_deploy",
    "Preview or queue an atomic development Project deployment.",
    {
      dryRun: booleanField("Preview without queueing", true),
      expectedPlanChecksum: optionalString("Checksum returned by the preview"),
    },
    false,
  ),
];
  ...
  app.route({
    ...
    handler: async (request, reply) => {
      ...
      if (request.method === "GET")
        return reply.status(405).header("allow", "POST, DELETE").send();
      ...
      if (rpc.method === "initialize") {
        if (!InitializeRequestSchema.safeParse(rpc).success)
          return reply
            .status(400)
        ...
        return rpcResult(rpc, {
          ...
          serverInfo: {
            name: "MCP Ops Studio Platform",
            version: "0.1.0",
            description: "Developer control-plane tools for MCP Ops Studio",
          },
        });
      }
      if (!sessionId)
        return reply
          .status(400)
          .send(rpcError(rpc.id ?? null, -32000, "Missing MCP session ID"));
      ...
      if (!CallToolRequestSchema.safeParse(rpc).success)
        return reply
          .status(400)
          .send(rpcError(rpc.id ?? null, -32602, "Invalid tools/call parameters"));
      ...
      try {
        ...
        return rpcResult(rpc, {
          isError: true,
          content: [{ type: "text", text: value.message ?? "Tool execution failed" }],
          structuredContent: {
            ok: false,
            error: {
              code: value.code ?? "TOOL_ERROR",
              message: value.message ?? "Tool execution failed",
            },
          },
        });
      }
...
async function callTool(
  ...
  if (name === "projects_list") {
    ...
    return output(
      "Active projects",
      { projects, selectedProjectId: session.projectId },
      [
        {
          tool: "project_select",
          reason: "Select a project before using project-scoped tools.",
        },
      ],
    );
  }
  ...
  if (!projectId)
    throw toolError(
      ...
      "Call project_select before using this tool.",
    );
  ...
  if (name === "project_get") {
    const environments = await prisma.environment.findMany({
      ...
      select: {
        ...
        slug: true,
        baseUrl: true,
        activeProjectDeploymentId: true,
      },
      orderBy: { name: "asc" },
    });
  ...
  if (name === "functions_list") {
    ...
    const functions = (await projectRepository(projectId).functions())
      .filter(
        (fn) =>
          !query ||
          `${fn.name} ${fn.slug} ${fn.description}`.toLowerCase().includes(query),
      )
      .map((fn) => safeFunction(fn));
    return output(`${functions.length} Function(s)`, { functions }, [
      { tool: "function_get", reason: "Read source and checksum before editing." },
    ]);
  }
  if (name === "function_get") {
    ...
    return output(
      ...
      {
        function: {
          ...safeFunction(fn, true),
          checksum: latest?.checksum ?? checksum(fn.code),
        },
      },
      [
        {
          tool: "function_edit",
          reason: "Use expectedVersion and expectedChecksum for a guarded edit.",
        },
      ],
    );
  }
  ...
  if (name === "library_get") {
    ...
    return output(`${library.name} v${library.version}`, {
      library,
      checksum: checksum(library.code),
    });
  }
  ...
  if (name === "endpoints_list") {
    ...
    return output(`${endpoints.length} endpoint(s)`, {
  ...
  if (name === "development_deploy")
    return developmentDeploy(projectId, identity, args);
...
function optionalObject(description: string) {
  return objectField(description);
}
...
function toolResult(value: Record<string, unknown>) {
  return {
    content: [{ type: "text", text: String(value.summary ?? "Completed") }],
    structuredContent: value,
  };
}
function output(summary: string, data: unknown, nextActions: unknown[] = []) {
...
function requireScope(identity: McpIdentity, scope: PlatformScope) {
  if (!identity.scopes.includes(scope))
    throw toolError("INSUFFICIENT_SCOPE", `OAuth scope ${scope} is required`, 403);
}
function requireRole(identity: McpIdentity, roles: string[]) {
  if (!roles.includes(identity.role))
    throw toolError(
      "FORBIDDEN",
      "The platform role does not allow this operation",
      403,
    );
}
async function loadSession(id: string): Promise<McpSession | null> {
  const raw = await controlPlaneState.get(`mcp:session:${id}`);
  return raw ? (JSON.parse(raw) as McpSession) : null;
}
async function saveSession(id: string, session: McpSession) {
...
async function findLibrary(projectId: string, identifier: string) {
  const library = await prisma.projectLibrary.findFirst({
    where: { projectId, OR: [{ id: identifier }, { importPath: identifier }] },
...
async function latestLibraries(projectId: string) {
  const rows = await prisma.projectLibrary.findMany({
    where: { projectId },
    orderBy: { version: "desc" },
  });
  return [...new Map(rows.map((row) => [row.importPath, row])).values()];
}
...
async function createFunction(
  ...
  const draft = functionCreateSchema.parse(args.draft);
  const validation = await validateFunction(projectId, draft);
  if (draft.secretGrantIds.length)
    throw toolError(
      "SECRETS_OUT_OF_SCOPE",
      "Platform MCP cannot assign Secret grants in this release",
    );
  if (!validation.valid)
    return { ...output("Function validation failed", { validation }), dryRun };
  if (dryRun)
    return {
      ...output(
        "Function creation preview",
        { draft: { ...draft, code: undefined }, validation },
        [
          {
            tool: "function_create",
            arguments: { dryRun: false },
            reason: "Apply this validated Function draft.",
          },
        ],
      ),
      dryRun: true,
  ...
  const fn = await prisma.$transaction(async (tx) => {
    const created = await tx.function.create({
      data: { projectId, ...draft, secretGrantIds: undefined, version: 1 } as never,
    });
    ...
    await tx.auditEvent.create({
      data: {
        ...
        targetType: "function",
        targetId: created.id,
        metadata: { source: "platform_mcp", version: 1 },
      },
    });
    return created;
  });
  return {
    ...output(`Created ${fn.name} v1`, {
      function: fn,
      checksum: checksum(draft.code),
    }),
    dryRun: false,
...
async function editFunction(
  ...
  if (
    fn.version !== input.expectedVersion ||
    currentChecksum !== input.expectedChecksum
  )
    throw toolError(
      "EDIT_CONFLICT",
  ...
  if (!validation.valid)
    return {
      ...output("Function edit validation failed", { validation }),
      dryRun: input.dryRun,
    };
  ...
  if (input.dryRun)
    return {
      ...output("Function edit preview", { diff, validation }, [
        {
          tool: "function_edit",
          reason: "Repeat with dryRun=false and the same version/checksum to apply.",
        },
      ]),
      dryRun: true,
      diff,
  ...
  await prisma.$transaction(async (tx) => {
    const updated = await tx.function.updateMany({
      where: { id: fn.id, projectId, version: input.expectedVersion },
      data: { ...input.changes, code, version: nextVersion } as never,
    });
    if (!updated.count)
      throw toolError("EDIT_CONFLICT", "Function changed while applying the edit", 409);
    if (code !== fn.code)
      await tx.functionVersion.create({
        data: {
          functionId: fn.id,
    ...
    await tx.auditEvent.create({
      data: {
        ...
        targetType: "function",
        targetId: fn.id,
        metadata: { source: "platform_mcp", ...diff },
      },
    });
  });
  return {
    ...output(`Saved ${fn.name} v${nextVersion}`, { diff, validation }),
    dryRun: false,
    diff,
  };
}
...
async function testFunction(
  ...
  if (dryRun || !validation.valid)
    return {
      ...output(
        dryRun
          ? "Function test preview; user code was not executed"
        ...
        dryRun
          ? [
              {
                tool: "function_test",
                reason: "Set dryRun=false only when real side effects are acceptable.",
              },
            ]
          : [],
      ),
  ...
  if (result.status >= 400)
    throw toolError("TEST_FAILED", `Runtime test failed with HTTP ${result.status}`);
...
async function editLibrary(
  ...
  const existing =
    typeof args.library === "string"
      ? await findLibrary(projectId, args.library)
      : null;
  const source =
    typeof args.source === "string"
      ? args.source
      : typeof args.patch === "string" && existing
        ? applyUnifiedPatch(existing.code, args.patch)
        : existing?.code;
  const draft = projectLibrarySchema.parse({
    ...(existing
      ? {
          name: existing.name,
          importPath: existing.importPath,
          description: existing.description,
          exportedFunctions: existing.exportedFunctions,
        }
      : object(args.draft)),
    code: source,
  });
  ...
  if (dryRun)
    return {
      ...output("Library version preview", { diff }, [
        {
          tool: "library_create_version",
          reason: "Repeat with dryRun=false to create the validated version.",
        },
      ]),
      dryRun: true,
      diff,
  ...
  const created = await prisma.projectLibrary.create({
    data: {
      ...
      exportedFunctions: draft.exportedFunctions ?? [],
      version: nextVersion,
    },
  });
  ...
  return {
    ...output(`Created ${created.importPath} v${created.version}`, {
      library: created,
    }),
    dryRun: false,
    diff,
...
async function createBinding(
  ...
  requireRole(identity, ["owner", "admin", "developer"]);
  const endpoint = await findEndpoint(projectId, z.string().parse(args.endpoint));
  const rawBinding = object(args.binding);
  const dryRun = args.dryRun !== false;
  const binding =
    endpoint.kind === "mcp"
      ? mcpBindingSchema.parse(rawBinding)
      : httpBindingSchema.parse(rawBinding);
  const functionId = binding.functionId;
  const fn = await prisma.function.findFirst({ where: { id: functionId, projectId } });
  if (!fn)
    throw toolError(
      "INVALID_BINDING_FUNCTION",
      "Binding Function does not belong to the selected project",
    );
  const isMcp = endpoint.kind === "mcp";
  const conflict = isMcp
    ? await prisma.mcpToolBinding.findFirst({
        where: {
          endpointId: endpoint.id,
          toolName: String("toolName" in binding ? binding.toolName : ""),
        },
      })
    : await prisma.httpRouteBinding.findFirst({
        where: {
          endpointId: endpoint.id,
          method: ("method" in binding ? binding.method : "GET") as never,
          path: String("path" in binding ? binding.path : ""),
        },
      });
  if (conflict)
    throw toolError("BINDING_CONFLICT", "An equivalent binding already exists", 409);
  if (dryRun)
    return {
      ...output("Binding creation preview", {
        endpoint: endpointSummary(endpoint),
        binding,
      }),
      dryRun: true,
  ...
  const created = isMcp
    ? await prisma.mcpToolBinding.create({
        data: {
          endpointId: endpoint.id,
          ...(binding as z.infer<typeof mcpBindingSchema>),
        },
      })
    : await prisma.httpRouteBinding.create({
        data: {
          endpointId: endpoint.id,
          ...(binding as z.infer<typeof httpBindingSchema>),
        } as never,
      });
  await prisma.auditEvent.create({
    data: {
      ...
      actorType: "user",
      actorId: identity.userId,
      action: isMcp ? "mcp_binding.created" : "http_binding.created",
      targetType: isMcp ? "mcp_tool_binding" : "http_route_binding",
      targetId: created.id,
      metadata: { source: "platform_mcp" },
    },
  });
...
async function editBinding(
  ...
  requireRole(identity, ["owner", "admin", "developer"]);
  const endpoint = await findEndpoint(projectId, z.string().parse(args.endpoint));
  const bindingId = z.string().uuid().parse(args.bindingId);
  const changes = object(args.changes);
  const dryRun = args.dryRun !== false;
  const existing =
    endpoint.kind === "mcp"
      ? await prisma.mcpToolBinding.findFirst({
          where: { id: bindingId, endpoint: { projectId, id: endpoint.id } },
        })
      : await prisma.httpRouteBinding.findFirst({
          where: { id: bindingId, endpoint: { projectId, id: endpoint.id } },
        });
  ...
  const candidate =
    endpoint.kind === "mcp"
      ? mcpBindingSchema.parse({ ...existing, ...changes })
      : httpBindingSchema.parse({ ...existing, ...changes });
  const ownedFunction = await prisma.function.findFirst({
    where: { id: candidate.functionId, projectId },
    select: { id: true },
  });
  if (!ownedFunction)
    throw toolError(
  ...
  const conflict =
    endpoint.kind === "mcp"
      ? await prisma.mcpToolBinding.findFirst({
          where: {
            endpointId: endpoint.id,
            toolName: (candidate as z.infer<typeof mcpBindingSchema>).toolName,
            id: { not: bindingId },
          },
        })
      : await prisma.httpRouteBinding.findFirst({
          where: {
            endpointId: endpoint.id,
            method: (candidate as z.infer<typeof httpBindingSchema>).method,
            path: (candidate as z.infer<typeof httpBindingSchema>).path,
            id: { not: bindingId },
          },
        });
  if (conflict)
  ...
  if (dryRun)
    return {
      ...output("Binding edit preview", { before: existing, after: candidate }),
      dryRun: true,
    };
  const updated =
    endpoint.kind === "mcp"
      ? await prisma.mcpToolBinding.update({
          where: { id: bindingId },
          data: candidate as never,
        })
      : await prisma.httpRouteBinding.update({
          where: { id: bindingId },
          data: candidate as never,
        });
  await prisma.auditEvent.create({
    data: {
      ...
      actorType: "user",
      actorId: identity.userId,
      action: endpoint.kind === "mcp" ? "mcp_binding.updated" : "http_binding.updated",
      targetType: endpoint.kind === "mcp" ? "mcp_tool_binding" : "http_route_binding",
      targetId: bindingId,
      metadata: { source: "platform_mcp", fields: Object.keys(changes) },
    },
  });
...
async function discoverEndpoint(projectId: string, args: Record<string, unknown>) {
  const endpoint = await findEndpoint(projectId, z.string().parse(args.endpoint));
  const urls = canonicalEnvironmentEndpointUrls(
    endpoint.project.environments,
    endpoint.project.slug,
    endpoint.slug,
  );
  const auth = endpoint.defaultAuthPolicy
    ? {
        type: endpoint.defaultAuthPolicy.type,
        config: endpoint.defaultAuthPolicy.config,
      }
    : null;
  const credential = credentialRequirement(auth);
  const formats = availableEndpointDocumentFormats(endpoint.kind);
  let document;
  if (args.format) {
    if (!formats.includes(args.format as EndpointDocumentFormat))
      throw toolError(
        "UNSUPPORTED_FORMAT",
        `Format ${args.format} is not available for this endpoint`,
      );
    document = generateEndpointDocument(args.format as EndpointDocumentFormat, {
      manifest: currentEndpointManifest(endpoint),
      environments: endpoint.project.environments.map((environment) => ({
        name: environment.name,
        slug: environment.slug,
        mcpUrl: urls[environment.slug]!.mcpUrl,
        httpBaseUrl: urls[environment.slug]!.httpBaseUrl,
      })),
  ...
  return output("Endpoint discovery", {
    endpoint: endpointSummary(endpoint),
    environments: urls,
    credential,
...
function credentialRequirement(auth: { type: string; config: unknown } | null) {
  if (!auth || auth.type === "public") return { type: "none", required: false };
  const config = object(auth.config);
  if (auth.type === "api_key")
    return {
      type: "api_key",
      required: true,
      in: "header",
      header: typeof config.header === "string" ? config.header : "x-api-key",
    };
  if (auth.type === "basic_auth")
    return { type: "basic", required: true, scheme: "Basic" };
  if (auth.type === "webhook_signature")
    return {
      type: "webhook_hmac",
      required: true,
      in: "header",
...
async function deploymentStatus(projectId: string) {
  const environment = await prisma.environment.findFirst({
    where: { projectId, slug: "development" },
    include: { activeProjectDeployment: true },
  });
  const inProgress = environment
  ...
  return output("Development deployment status", {
    environment: environment
      ? {
          id: environment.id,
          activeProjectDeploymentId: environment.activeProjectDeploymentId,
        }
      : null,
...
async function developmentDeploy(
  ...
  requireScope(identity, "mcpops:deploy");
  requireRole(identity, ["owner", "admin", "developer", "operator"]);
  const plan = await developmentDeploymentPlan(projectId);
  const dryRun = args.dryRun !== false;
  if (dryRun)
    return {
      ...output("Development deployment preview", plan, [
        {
          tool: "development_deploy",
          arguments: { dryRun: false, expectedPlanChecksum: plan.planChecksum },
          reason: "Queue this exact project plan.",
        },
      ]),
      dryRun: true,
    };
  if (args.expectedPlanChecksum !== plan.planChecksum)
    throw toolError(
      "DEPLOYMENT_PLAN_CHANGED",
      "Project state changed after preview; run a new dry run",
      409,
    );
  const deployment = await queueDevelopmentDeployment({
    userId: identity.userId,
    projectId,
    role: identity.role,
    email: identity.email,
  ...
  return {
    ...output(`Development v${deployment.version} queued`, {
      deployment,
