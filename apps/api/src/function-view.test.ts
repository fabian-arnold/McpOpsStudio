      ...
      [
        {
          ...
          method: "POST",
          path: "/search",
          inputMapping: { query: "body.query" },
          enabled: false,
          endpoint: { ...endpoint, kind: "http" },
        },
      ],
    );

    expect(result.mcpBindings[0]).toMatchObject({
      toolName: "search",
      endpoint: { id: "endpoint-1", kind: "mcp" },
    });
    expect(result.httpBindings[0]).toMatchObject({
      method: "POST",
      path: "/search",
      inputMapping: { query: "body.query" },
