...
export const executionListQuerySchema = z.object({
  ...
  status: z.enum(["success", "error", "denied", "timeout", "validation_error"]).optional(),
...
export const deploymentListQuerySchema = z.object({
  ...
  status: z.enum(["queued", "building", "deploying", "active", "failed", "rolled_back"]).optional(),
  format: z.enum(["json", "csv"]).default("json"),
}).strict().superRefine(validDateRange);
...
function validDateRange(value: { from?: Date; to?: Date }, context: z.RefinementCtx): void {
  if (value.from && value.to && value.from > value.to) context.addIssue({ code: z.ZodIssueCode.custom, path: ["to"], message: "The end date must be after the start date" });
}
