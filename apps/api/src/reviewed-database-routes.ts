...
export async function registerReviewedDatabaseRoutes(
  app: FastifyInstance,
): Promise<void> {
  app.get("/api/database/connections", async (request) => {
    const session = admin(request);
    const { environmentId } = parse(
      listQuerySchema.omit({ connectionId: true }),
      request.query,
    );
    const rows = await prisma.databaseConnection.findMany({
      ...
      include: {
        ...
        _count: { select: { queryDefinitions: true } },
      },
  ...
  app.post("/api/database/connections", async (request, reply) => {
    ...
    if (!environment || !secret)
      return notFound(reply, request, "Environment or connection secret not found");
    const created = await prisma.$transaction(async (tx) => {
      const row = await tx.databaseConnection.create({
        data: { projectId: session.projectId, ...input },
      });
      await tx.auditEvent.create({
        data: {
          ...
          targetType: "database_connection",
          targetId: row.id,
          metadata: { name: row.name, secretId: row.secretId },
        },
      });
      return row;
    });
    return reply.status(201).send(
      connectionView({
        ...
        environment,
        secret,
        _count: { queryDefinitions: 0 },
      }),
    );
  });

  app.post(
    "/api/database/connections/:connectionId/disable",
    async (request, reply) => {
      const session = admin(request);
      ...
      const row = await prisma.$transaction(async (tx) => {
        ...
        await tx.auditEvent.create({
          data: {
            ...
            actorType: "user",
            actorId: session.userId,
            action: "database_connection.disabled",
            targetType: "database_connection",
  ...
  app.post("/api/database/queries", async (request, reply) => {
    ...
    if (!connection)
      return notFound(
        reply,
        request,
        "Enabled database connection not found in this environment",
      );
    const { environmentId, connectionId, queryId, name, description, ...versionInput } =
      input;
    const definition = await prisma.$transaction(async (tx) => {
      const created = await tx.reviewedQueryDefinition.create({
        data: {
          projectId: session.projectId,
          environmentId,
          connectionId,
      ...
      const version = await tx.reviewedQueryVersion.create({
        data: {
          queryDefinitionId: created.id,
          version: 1,
          ...versionInput,
          createdByUserId: session.userId,
        } as Prisma.ReviewedQueryVersionUncheckedCreateInput,
      });
      await tx.auditEvent.create({
        data: {
          projectId: session.projectId,
          ...
          actorType: "user",
          actorId: session.userId,
          action: "reviewed_query.created",
          targetType: "reviewed_query_definition",
          targetId: created.id,
          metadata: auditQueryMetadata(created.queryId, version),
        },
      });
      return tx.reviewedQueryDefinition.findUniqueOrThrow({
        where: { id: created.id },
        include: queryDefinitionInclude,
      });
    });
  ...
  app.post(
    "/api/database/queries/:queryDefinitionId/versions",
    async (request, reply) => {
      const session = admin(request);
      ...
      validateVersionInput(input);
      const definition = await prisma.reviewedQueryDefinition.findFirst({
        where: { id: queryDefinitionId, projectId: session.projectId },
        include: { connection: true },
      });
      if (!definition)
        return notFound(reply, request, "Reviewed query definition not found");
      if (!definition.connection.enabled)
        throw failure(
          409,
          "DATABASE_CONNECTION_DISABLED",
          "Cannot create a query version for a disabled connection",
        );
      const latest = await prisma.reviewedQueryVersion.aggregate({
        where: { queryDefinitionId },
        _max: { version: true },
      });
      const created = await prisma.$transaction(async (tx) => {
        const version = await tx.reviewedQueryVersion.create({
        ...
        await tx.auditEvent.create({
          data: {
            ...
            actorType: "user",
            actorId: session.userId,
            action: "reviewed_query.version_created",
            targetType: "reviewed_query_version",
            targetId: version.id,
            metadata: auditQueryMetadata(definition.queryId, version),
          },
        });
        return version;
      });
  ...
  app.post(
    "/api/database/query-versions/:queryVersionId/disable",
    async (request, reply) => {
      const session = admin(request);
      const { queryVersionId } = request.params as { queryVersionId: string };
      const existing = await prisma.reviewedQueryVersion.findFirst({
        where: {
          id: queryVersionId,
          queryDefinition: { projectId: session.projectId },
        },
        include: { queryDefinition: true },
      });
      if (!existing)
        return notFound(reply, request, "Reviewed query version not found");
      const updated = await prisma.$transaction(async (tx) => {
        const version = await tx.reviewedQueryVersion.update({
          where: { id: queryVersionId },
          data: { enabled: false },
        });
        await tx.auditEvent.create({
          data: {
            projectId: session.projectId,
            environmentId: existing.queryDefinition.environmentId,
            actorType: "user",
            actorId: session.userId,
            action: "reviewed_query.version_disabled",
            targetType: "reviewed_query_version",
            targetId: version.id,
            metadata: {
              queryId: existing.queryDefinition.queryId,
  ...
  app.get(
    "/api/functions/:functionId/database-query-grants",
    async (request, reply) => {
      const session = admin(request);
      ...
      const grants = await prisma.functionQueryGrant.findMany({
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const [fn, version] = await Promise.all([
        ...
        prisma.reviewedQueryVersion.findFirst({
          where: {
            id: queryVersionId,
            enabled: true,
            queryDefinition: { projectId: session.projectId },
          },
          include: { queryDefinition: { include: { connection: true } } },
        }),
      ]);
      if (!fn || !version)
        return notFound(
          reply,
          request,
          "Function or enabled reviewed query version not found",
        );
      if (!version.queryDefinition.connection.enabled)
        throw failure(
          409,
          "QUERY_CONNECTION_DISABLED",
          "The reviewed query connection is disabled",
        );
      const grant = await prisma.$transaction(async (tx) => {
        const row = await tx.functionQueryGrant.upsert({
          where: {
            functionId_queryDefinitionId: {
              functionId,
              queryDefinitionId: version.queryDefinitionId,
            },
          },
          create: {
            functionId,
        ...
        await tx.auditEvent.create({
          data: {
            projectId: session.projectId,
            environmentId: version.queryDefinition.environmentId,
            functionId,
            actorType: "user",
            actorId: session.userId,
            action: "reviewed_query.granted",
            targetType: "function_query_grant",
            targetId: row.id,
            metadata: {
              queryId: version.queryDefinition.queryId,
              queryVersionId,
              version: version.version,
              connectionId: version.queryDefinition.connectionId,
            },
          },
        });
        return tx.functionQueryGrant.findUniqueOrThrow({
          where: { id: row.id },
          include: grantInclude,
        });
      });
  ...
  app.delete(
    "/api/functions/:functionId/database-query-grants/:grantId",
    async (request, reply) => {
      const session = admin(request);
      const { functionId, grantId } = request.params as {
        functionId: string;
        grantId: string;
      };
      const [fn, grant] = await Promise.all([
        ownedFunction(session, functionId),
        prisma.functionQueryGrant.findFirst({
          where: {
            id: grantId,
            functionId,
            function: { projectId: session.projectId },
          },
          include: { queryDefinition: true, queryVersion: true },
        }),
      ]);
      if (!fn || !grant)
        return notFound(reply, request, "Function query grant not found");
      await prisma.$transaction([
        prisma.functionQueryGrant.update({
          where: { id: grantId },
          data: { enabled: false },
        }),
        prisma.auditEvent.create({
          data: {
            projectId: session.projectId,
            environmentId: grant.queryDefinition.environmentId,
            functionId,
            actorType: "user",
            actorId: session.userId,
            action: "reviewed_query.revoked",
            targetType: "function_query_grant",
            targetId: grantId,
            metadata: {
              queryId: grant.queryDefinition.queryId,
              queryVersionId: grant.queryVersionId,
              version: grant.queryVersion.version,
            },
          },
        }),
      ]);
...
function queryDefinitionView(row: {
  id: string;
