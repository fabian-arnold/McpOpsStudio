...
export function issueSession(
  reply: FastifyReply,
  session: Omit<PlatformSession, "expiresAt">,
): string {
  const csrf = randomBytes(24).toString("base64url");
  const secure = process.env.COOKIE_SECURE === "true";
  reply.setCookie(
    "mcpops_session",
    encodeSession({ ...session, expiresAt: Date.now() + 8 * 60 * 60 * 1000 }),
    { httpOnly: true, secure, sameSite: "strict", path: "/", maxAge: 28_800 },
  );
  reply.setCookie("mcpops_csrf", csrf, {
    httpOnly: false,
    secure,
    sameSite: "strict",
    path: "/",
...
export function clearSession(reply: FastifyReply): void {
  reply.clearCookie("mcpops_session", { path: "/" });
  reply.clearCookie("mcpops_csrf", { path: "/" });
}
