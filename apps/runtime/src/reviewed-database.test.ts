...
const definition: ReviewedQuerySnapshot = {
  ...
  connection: {
    id: "connection-1",
    name: "analytics",
    secretId: "secret-connection-1",
  },
  sql: "SELECT id, name FROM customers WHERE tenant_id = $1 AND name ILIKE $2",
  parameterOrder: ["tenantId", "query"],
  parameterSchema: {
    type: "object",
...
function harness(
  overrides: Partial<ConstructorParameters<typeof SnapshotReviewedDatabase>[0]> = {},
) {
  const events: unknown[] = [];
  const logs: unknown[] = [];
  const adapter: ReviewedQueryAdapter = {
    execute: vi.fn(async () => [{ id: "1" }, { id: "2" }, { id: "3" }]),
  };
  const logger: SafeLogger = {
    debug: vi.fn(),
    info: vi.fn((message, metadata) => logs.push({ message, metadata })),
    warn: vi.fn((message, metadata) => logs.push({ message, metadata })),
    error: vi.fn(),
  };
  ...
  const resolveConnectionSecret = vi.fn(
    async () => "postgresql://reviewed:super-secret@db.example.test/analytics",
  );
  const db = new SnapshotReviewedDatabase({
    enabled: true,
    functionId: "function-1",
...
describe("snapshot reviewed database capability", () => {
  it("validates immutable reviewed-query snapshots with the shared PostgreSQL AST policy", () => {
    const base = {
      functions: [],
      reviewedQueries: [definition],
      capabilities: { reviewedDatabaseQueries: { enabled: true } },
    };
    expect(deploymentSnapshotSchema.parse(base).reviewedQueries[0]?.queryId).toBe(
      "customers.search",
    );
    expect(() =>
      deploymentSnapshotSchema.parse({
        ...base,
        reviewedQueries: [
          { ...definition, sql: "DELETE FROM customers WHERE tenant_id = $1" },
        ],
      }),
    ).toThrow();
  });
  ...
  it("requires both the explicit environment flag and deployment capability", () => {
    expect(reviewedQueriesEnabled({ ENABLE_REVIEWED_DB_QUERIES: "true" }, true)).toBe(
      true,
    );
    expect(reviewedQueriesEnabled({ ENABLE_REVIEWED_DB_QUERIES: "TRUE" }, true)).toBe(
      false,
    );
    expect(reviewedQueriesEnabled({ ENABLE_REVIEWED_DB_QUERIES: "true" }, false)).toBe(
      false,
    );
  });
  ...
  it("executes only the immutable granted query and caps returned rows", async () => {
    ...
    await expect(
      db.query({
        ...
        queryId: "customers.search",
        params: { tenantId: "acme", query: "%ada%" },
      }),
    ).resolves.toEqual({
      rows: [{ id: "1" }, { id: "2" }],
      rowCount: 2,
    ...
    expect(telemetry).not.toContain("SELECT");
    expect(telemetry).not.toContain("%ada%");
  });

  it("rejects an ungranted connection before resolving any credential", async () => {
    const { db, adapter, resolveConnectionSecret } = harness();
    await expect(
      db.query({ connection: "production", queryId: "customers.search", params: {} }),
    ).rejects.toMatchObject({ code: "FORBIDDEN", requestId: "request-1" });
  ...
  it("fails closed while the runtime feature is disabled", async () => {
    ...
    await expect(
      db.query({ connection: "analytics", queryId: "customers.search", params: {} }),
  ...
  it("maps database cancellation to a safe timeout and never exposes the driver error", async () => {
    const adapter: ReviewedQueryAdapter = {
      execute: vi.fn(async () => {
        throw Object.assign(new Error("password super-secret failed"), {
          code: "57014",
        });
      }),
    };
  ...
  it("caps oversized result payloads without returning a partial oversized row", async () => {
    const adapter: ReviewedQueryAdapter = {
      execute: vi.fn(async () => [{ payload: "x".repeat(2_000) }]),
    };
    const { db } = harness({
      adapter,
      definitions: [{ ...definition, maxRows: 10, maxBytes: 1_024 }],
    });
    await expect(
      db.query({
        connection: "analytics",
    ...
    ).resolves.toEqual({ rows: [], rowCount: 0, truncated: true });
  });
});
...
describe("PostgreSQL reviewed query adapter", () => {
  it("uses a read-only transaction, server timeout, positional parameters, and a server-side row cap", async () => {
    ...
    const client = {
      ...
      query: vi.fn(async (input: string | { text: string; values?: unknown[] }) => {
        calls.push(input);
        return {
          rows:
            typeof input === "object" && input.text.startsWith("SELECT * FROM")
              ? [{ id: "1" }]
              : [],
        };
      }),
    ...
    expect(calls[2]).toEqual({
      text: 'SELECT * FROM (SELECT id FROM customers WHERE tenant_id = $1) AS "__mcpops_reviewed_query" LIMIT $2',
      values: ["acme", 26],
    });
  ...
  it("destroys the active PostgreSQL client when invocation cancellation is signalled", async () => {
    ...
    const client = {
      connect: vi.fn(async () => undefined),
      query: vi.fn((input: string | { text: string; values?: unknown[] }) => {
        if (typeof input === "object" && input.text.startsWith("SELECT * FROM")) {
          startQuery();
          return new Promise<{ rows: Record<string, unknown>[] }>(() => undefined);
        }
