...
export async function checkRuntimeReadiness(
  ...
  const [postgres, redis, deployments] = await Promise.allSettled([
    withTimeout(dependencies.postgres(), timeoutMs),
    withTimeout(dependencies.redis(), timeoutMs),
    withTimeout(dependencies.activeDeployments(), timeoutMs),
  ]);
  const checks = {
    postgres: postgres.status === "fulfilled",
    redis: redis.status === "fulfilled",
    activeDeployments: deployments.status === "fulfilled",
  };
  return {
