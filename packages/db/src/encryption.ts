...
function decodeMasterKey(encoded: string | undefined): Buffer {
  ...
  if (key.length !== 32) throw new Error("MCP_OPS_MASTER_KEY must decode to exactly 32 bytes.");
  return key;
}

/** AES-256-GCM produces authenticated ciphertext; no plaintext secret is persisted. */
export function encryptSecret(plaintext: string, encodedKey = process.env.MCP_OPS_MASTER_KEY): string {
...
export function decryptSecret(payload: string, encodedKey = process.env.MCP_OPS_MASTER_KEY): string {
  ...
  const decipher = createDecipheriv("aes-256-gcm", decodeMasterKey(encodedKey), Buffer.from(ivValue, "base64url"));
