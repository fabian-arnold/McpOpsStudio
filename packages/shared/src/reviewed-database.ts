...
const identifier = z
  .string()
  .regex(/^[a-z][a-z0-9_]{0,62}$/, "Use a lowercase SQL-safe identifier");
const parameterName = z
  .string()
  .regex(/^[A-Za-z_][A-Za-z0-9_]{0,127}$/, "Use a valid parameter name");
const logicalQueryId = z
  .string()
  .max(256)
  .regex(
    /^[a-z][a-z0-9_]*(?:\.[a-z][a-z0-9_]*)*$/,
    "Use a lowercase dotted logical query ID",
  );
const jsonSchemaObject = z.record(z.unknown()).superRefine((value, context) => {
  if (value.type !== "object")
    context.addIssue({
      code: z.ZodIssueCode.custom,
      message: "Parameter schema must be a JSON Schema object with type: object",
    });
});

export const reviewedDatabaseConnectionCreateSchema = z
  .object({
...
export const reviewedQueryVersionCreateSchema = z
  .object({
...
export const reviewedQueryDefinitionCreateSchema = reviewedQueryVersionCreateSchema
  .extend({
    environmentId: z.string().uuid(),
...
export const functionQueryGrantCreateSchema = z
  .object({ queryVersionId: z.string().uuid() })
  .strict();
 ...
 */
export function validateReviewedReadQuery(
  sql: string,
  parameterOrder: readonly string[],
): ReviewedSqlValidation {
  ...
  if (statements.length !== 1)
    throw new Error("Reviewed queries must contain exactly one statement");
  ...
  walk(statements[0], (node) => {
    if (node.type === "parameter") {
      ...
      if (!match || Number(match[1]) < 1)
        throw new Error(
          "Reviewed queries may use only positional PostgreSQL parameters",
        );
    ...
    if (node.type === "call") {
      ...
      const functionName =
        typeof qname.name === "string" ? qname.name.toLowerCase() : "";
      const schema = typeof qname.schema === "string" ? qname.schema.toLowerCase() : "";
      if (
        disallowedFunctions.has(functionName) ||
        disallowedSchemas.has(schema) ||
        functionName.startsWith("dblink_") ||
        functionName.startsWith("pg_") ||
        functionName.startsWith("lo_")
      ) {
        throw new Error(
          `Function '${functionName || "unknown"}' is not allowed in reviewed queries`,
        );
      }
    }
    if (
      node.type === "keyword" &&
      identityKeywords.has(String(node.keyword).toLowerCase())
    ) {
  ...
  if (
    ...
    throw new Error(
      "SQL positional parameters must match parameterOrder exactly ($1 through $N)",
    );
  }
  return { parameterPositions: ordered };
}

export function validateReviewedParameterSchema(
  parameterOrder: readonly string[],
  ...
  if (
    declared.length !== ordered.length ||
    declared.some((name, index) => name !== ordered[index])
  ) {
    throw new Error(
      "parameterOrder must name every parameterSchema property exactly once",
    );
  }
}
...
function assertReadOnlySelect(value: unknown): void {
  ...
  if (statement.type === "select") {
    if (statement.for !== undefined && statement.for !== null)
  ...
  throw new Error(
    "Reviewed queries must be a read-only SELECT; DDL, DML and transaction statements are forbidden",
  );
}
