...
export function parseMasterKey(encoded: string | undefined): Buffer {
  ...
  if (key.length !== 32) throw new Error("MCP_OPS_MASTER_KEY must encode exactly 32 bytes (64 hex characters or base64)");
  return key;
}
...
export function decryptSecret(payload: string, key: Buffer): string {
  ...
  if (version !== VERSION || !iv || !tag || !encrypted || extra.length) throw new Error("Encrypted secret has an invalid format");
  try {
