...
lines.on("line", (line) => {
  try {
    handle(JSON.parse(line) as ExecuteMessage | CancelMessage | RpcResult);
  } catch {
    send({
      type: "error",
      error: { code: "INTERNAL_ERROR", message: "Invalid executor protocol message" },
    });
  }
});
...
function handle(raw: ExecuteMessage | CancelMessage | RpcResult): void {
  if (raw.type === "rpc-result" || raw.type === "rpc-error") {
    ...
    if (raw.type === "rpc-result") item.resolve(raw.value);
      ...
      item.reject(
        Object.assign(new Error(raw.error?.message ?? "Capability failed"), raw.error),
      );
    return;
  }
  if (raw.type === "cancel") {
...
function createContext(
  ...
  const rpc = (operation: string, ...args: unknown[]) =>
    new Promise<unknown>((resolve, reject) => {
      ...
      send({ type: "rpc", id, operation, args });
    });
  ...
  return Object.freeze({
    ...
    audit: Object.freeze({ write: (event: unknown) => rpc("audit.write", event) }),
    db: Object.freeze({ query: (request: unknown) => rpc("db.query", request) }),
    collections: Object.freeze({ collection }),
    abortSignal,
  });
}
function send(value: unknown): void {
  hostProcess.stdout.write(JSON.stringify(value) + "\n");
}
send({ type: "ready" });
